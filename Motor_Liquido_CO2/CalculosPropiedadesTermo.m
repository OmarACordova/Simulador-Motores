...
RolOx=RolOx_c*exp(b1_RolOx*(1-Trl)^(1/3) + b2_RolOx*(1-Trl)^(2/3) + b3_RolOx*(1-Trl) + b4_RolOx*(1-Trl)^(4/3));         % cambiar
...
Cvl = b1_Cpl*exp( b2_Cpl *TlOx(i)) + (b3_Cpl)*exp(b4_Cpl  *TlOx(i));

Pvapl(i)=b1_PvapT*TlOx(i).^3 + b2_PvapT*TlOx(i).^2 + b3_PvapT*TlOx(i) + b4_PvapT;
Pvapg(i)=b1_PvapT*TgOx(i).^3 + b2_PvapT*TgOx(i).^2 + b3_PvapT*TgOx(i) + b4_PvapT;

Rtank=Xtank/Ktank;
Rov=mv(i)./VgOx(i);

Cpvg=((Cvv)*mv(i)+(Cvg+Rg).*mgOx(i))./(mgOx(i)+mv(i));
...
if Pv(i)>0.5*10^6
    Tf(i)=b1_TPvap*exp(b2_TPvap*Pv(i)) +  b3_TPvap*exp(b4_TPvap*Pv(i));
else
    Tf(i)=TfMin;
end
