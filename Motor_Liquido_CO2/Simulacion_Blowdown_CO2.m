...
Pv(1)=3*10^6          % Presion de los vapores de oxido nitroso Depende de la operacion
PgOx(1)=3*10^6            % Presion inicial del prezurizante en el tanque del Oxidante operacion
PgFu(1)=6*10^6            % Presion inicial del presurizante en el tanque del combustible operacion

TlOx(1)=273+0;         % Temperatura de Oxidante liquido % Variable de Diseño %Sustancia
...
mlFu(1)=3.3             % Masa de combustible % Variable de Diseño
VlOx(1)=19*10^-3        % Volumen inicial del liquido del oxidante % Variable de Diseño
UllageOx=0.65
UllageFu=1.1

MaxOpPressProp=7*10^6
MaxOpPressPres=31*10^6
MaxOpPressChamber=4*10^6
PgPr(1)=MaxOpPressPres         % Presion inicial del presurizante en el tanque del presuriaznante operacion
VlFu(1)=mlFu(1)/RolFu   % Volumen inicial del liquido del combustible % Variable de Diseño
...
DVent=(1/4)*0.0254;         % Variable de Diseño
...
VgOx(1)=VlOx(1)*UllageOx         % Variable de Diseño
VgPr=15*10^-3;           % Variable de Diseño
VgFu(1)=VlFu*UllageFu        % Variable de Diseño
TgOx(1)=Tatm
TgPr(1)=Tatm
TgFu(1)=Tatm
...
Trg=TgOx(1)/Tcrita            % Temepratrua reducida del los vapores del oxidnate% %Sustancia
Trl=TlOx(1)/Tcrita            % Temepratura reducuida de los liquidos del oxidnante %Sustancia

% Densidad liquida del Oxidante en funcion de la temperatura
RolOx=RolOx_c*exp(b1_RolOx*(1-Trl)^(1/3) + b2_RolOx*(1-Trl)^(2/3) + b3_RolOx*(1-Trl) + b4_RolOx*(1-Trl)^(4/3)) %Sustancia
...
% inicializar varibles complemtarias
dQgwOx(1)=0;
dQgfOx(1)=0;
dQlfOx(1)=0;
dQlwOx(1)=0;
...
%%
MODO=5;
while(tiempo<11*60 && mlOx(i)>=0 && mlFu(i)>=0)
    ...
    if MODO==4 || MODO==5
        ...
        for n=1:10
            ...
            %[1 PtankOx(i)/1000 PgFu(i)/1000 Pc(i)/1000 dmLineOx(i+1) dmLineFu(i+1) OFR(i)]
            ...
            To_NOS_ETH(i) = (To_1*OFR(i).^2 + To_2*OFR(i) + To_3)./(OFR(i).^3 + To_4*OFR(i).^2 + To_5*OFR(i) + To_6);

            % Masa Molar
            Mm_NOS_ETH(i) = (Mm_1*OFR(i).^2 +    Mm_2.*OFR(i) + Mm_3)./ (OFR(i).^2 + Mm_4.*OFR(i) + Mm_5);
            % Gamma
            Gamma_NOS_ETH(i)=(Gamma_1.*OFR(i).^3 + Gamma_2.*OFR(i).^2 + Gamma_3.*OFR(i) + Gamma_4)./(OFR(i).^2 + Gamma_5.*OFR(i) + Gamma_6);
            ...
            %[2 PtankOx(i) PgFu(i) Pc(i) dmLineOx(i+1) dmLineFu(i+1) To_NOS_ETH(i) Mm_NOS_ETH(i) Gamma_NOS_ETH(i) Char_Velocity(i)]
        end
        ...
        Mach_Guess=1;
        Exp_Ratio_Guess=1;
        while Exp_Ratio_Guess<Exp_Ratio && MODO~=6
    ...
    if MODO==6
        Pc(i)=Patm;
        [dmLineOx(i+1) dmLineFu(i+1)]=MassFlow_InjLine(PtankOx(i), PgFu(i), Pc(i), TlOx(i), Pumbling);
        VLineOx=dmLineOx(i+1)/(ALineOx*RolOx);
        VLineFu=dmLineFu(i+1)/(ALineFu*RolFu);
        Mach(i)=0;
        Pexit(i)=Patm;
...
plot(time(1:end-1), Pvapl, time(1:end-1), Pvapg, time, PgOx)
